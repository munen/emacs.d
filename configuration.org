...
2. Count the added or removed TODO statements with =git log --since=yesterday -p things.org | grep TODO | grep -E "^\+|^\-" | wc -l=
3. Aggregate and print.

#+begin_src emacs-lisp
  ;; Std. Elisp way, including some mutable state, but fully functional:

  ;;; (defun productivity-of-the-day ()
  ;;;   (setq count 0)
  ;;;   (dolist (path org-agenda-files)
  ;;;     (let* ((folder (file-name-directory path))
  ;;;            (file (file-name-nondirectory path))
  ;;;            (cmd (concat "cd " folder "; git log --since=yesterday -p " file " | grep TODO | grep -E \"^\\+|^\\-\" | wc -l"))
  ;;;            (cnt (string-to-number (shell-command-to-string cmd))))
  ;;;       (setq count (+ count cnt))))
  ;;;   (print count))

  
  ;; Using a reducer, but adding dash.el as dependency.
