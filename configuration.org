 ...
 #+BEGIN_SRC emacs-lisp
   ...
   (defun mu4e-blacklist-from (msg)
     ...
     (let* ((from (mu4e-message-field msg :from))
            ...
            (path "~/.procmail/blacklist_from.txt"))
       ;; Blacklist the senders Name
       (if from_name
       ...
       (message "Blacklist: %s" from)))


   (defun mu4e-whitelist-from (msg)
     "Add the `from` of a message to the procmail whitelist"
     (let* ((from (mu4e-message-field msg :from))
            (from_name (car (car from)))
            (from_address (cdr (car from)))
            (path "~/.procmail/whitelist_from.txt"))
       ;; Whitelist the senders Name
       (if from_name
           (append-line-to-file from_name path))
       ;; Whitelist the Email-Address
       (append-line-to-file from_address path)
       (shell-command (format "sort -u -o %s %s" path path))
       (message "Whitelist: %s" from)))
   ...
   (add-to-list 'mu4e-headers-actions
                '("f Whitelist 'From:'" . mu4e-whitelist-from) t)
   ...
   ;; FIXME: Why isn't this working?
   ;; (add-to-list 'mu4e-headers-actions
   ;;   '("f White 'From:'" . (apply-partially 'mu4e-strategy-from "whitelist")) t)

   ;; (add-to-list 'mu4e-headers-actions
   ;;   '("F Block 'From:'" . (apply-partially 'mu4e-strategy-from "blacklist")) t)
