...
*** Autoformatting using prettier

Automatically format JavaScript code while honoring eslint
configuration. This is done using the [[https://github.com/prettier/prettier-eslint-cli][prettier-eslint-cli]] tool which
can be installed as a lone dependency which is nice, because then this
setup is completely local to the user and doesn't impose dependencies
to other team members.

This requires =prettier-eslint-cli= to be installed:
...
#+BEGIN_SRC emacs-lisp
  (evil-leader/set-key-for-mode 'js2-mode "f" 'prettier-eslint)
#+END_SRC
