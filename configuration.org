...
**** edit_file_non_interactive

#+begin_src emacs-lisp
  (defun munen-gptel--edit-file-non-interactive (file-path &optional file-edits)
    "Edit FILE-PATH by applying FILE-EDITS and save changes automatically (non-interactive).
  ...
  WARNING: This modifies files without confirmation - use with caution!"
    (if (and file-path (not (string= file-path "")) file-edits)
        (with-current-buffer (get-buffer-create "*edit-file-non-interactive*")
          (erase-buffer)
          ...
          (let ((inhibit-read-only t)
            ...
            (if edit-success
                (progn
                  ;; Save the buffer's contents to the original file
                  (write-file target-file-name nil)
                  (kill-buffer (current-buffer)) ; Clean up the temporary buffer
                  (format "Successfully edited and saved %s" target-file-name))
              (progn
                (kill-buffer (current-buffer)) ; Clean up the temporary buffer even on failure
                (format "Failed to apply edits to %s. File was not saved." target-file-name)))))
      (format "Failed to edit %s (invalid path or no edits provided)." file-path)))

  (gptel-make-tool
   :function #'munen-gptel--edit-file-non-interactive
   :name "edit_file_non_interactive"
   :description "Edit a file non-interactively by applying a list of edits and automatically saving changes.

  This tool modifies the target file directly on disk without user confirmation or review.
  Each edit specifies a line number, old string to find, and new string replacement.

  WARNING: This tool directly modifies and saves files without interactive review. Use with caution!
  All successful edits are applied and the file is saved automatically."
   :args (list '(:name "file-path"
               ...
               '(:name "file-edits"
                       ...
                       :items (:type object
                                     :properties
                                     (:line_number
                                      (:type integer :description "The line number (1-based) where the old_string is expected.")
                                      :old_string
                                      (:type string :description "The exact string to be replaced on the specified line.")
                                      :new_string
                                      (:type string :description "The new string to replace old_string.")))
                       :description "A list of edits to apply to the file. Each edit must contain line_number, old_string, and new_string."))
   :category "filesystem"
   :include t)
#+end_src
