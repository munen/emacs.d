                 ...
                 (with-temp-message "Executing command ..."
                   (shell-command-to-string
                    (concat script_program " "
                            ...
                            script_args))))
     :name "run_script"
     :description "Executes a user-specified script using a given program/interpreter and arguments. Returns the standard output of the script as a string. IMPORTANT: This tool allows execution of arbitrary code; user confirmation will be required before any script is run."
     :args (list
            '(:name "script_program"
                    :type string
                    :description "The program or interpreter to execute the script (e.g., 'python', 'bash', 'node').")
            '(:name "script_file"
                    :type string
                    :description "Path to the script file to execute. Supports relative paths from the user's home directory (e.g., using '~').")
            '(:name "script_args"
                    :type string
                    :description "Command-line arguments to be passed to the script. Provide as a single string. Can be an empty string if no arguments are needed."))
     :category "filesystem"
     :confirm t  ;; Crucial: This ensures gptel will ask for your confirmation
     :include t  ;; Optional: Consider if you want results automatically in the buffer
     )
...
#+begin_src emacs-lisp
  ...
  ;;  :description "Fetch content from a URL using trafilatura"
  ;;  :args '((:name "url"
  ;;                 :type string
