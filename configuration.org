...
Use UTF-8 consistently for file contents, file names, terminal input,
and terminal output. This avoids buffers being visited as raw bytes when
the Emacs daemon inherits an incomplete or non-UTF-8 locale.
...
Similar to =ps-print-buffer=, but exporting a PDF.

#+begin_src emacs-lisp
(defun print-buffer-to-pdf (filename)
  "Export the whole current buffer to PDF using `ps-print-buffer` output."
  (interactive
   (list
    (read-file-name
     "Write PDF file: "
  ...
  (unless (executable-find "ps2pdf")
    (user-error "Cannot find ps2pdf; install Ghostscript"))
  (let ((ps-file (make-temp-file "emacs-ps-print-" nil ".ps"))
        (pdf-file (expand-file-name filename)))
    (unwind-protect
        (progn
          (ps-print-buffer ps-file)
          (let ((status
                 (call-process "ps2pdf" nil "*ps2pdf*" nil
                               ps-file pdf-file)))
            (unless (zerop status)
              (pop-to-buffer "*ps2pdf*")
              (user-error "ps2pdf failed with exit status %s" status)))
          (message "Wrote %s" pdf-file))
      (when (file-exists-p ps-file)
        (delete-file ps-file)))))
#+end_src
...
(defun edge-tts-speak-region (start end)
  "Send the selected region to the local Edge TTS service."
  (interactive "r")
  (shell-command-on-region start end "~/bin/edge_tts"))
#+end_src
...
This will prevent warning buffers with lower severity than =:error= from
...
Even with these settings, you can manually view the output buffer when
compilation fails:

- Use =C-x b= *Org PDF LaTeX Output* to switch to the buffer
...
#+BEGIN_SRC emacs-lisp
    ...
    (setq things-file (expand-file-name "active_things.org" org-directory))
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; attached to.
  (setq ok-pomodoro-buffer "active_things.org")
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun set-org-agenda-files ()
    ...
    (setq org-agenda-files (flatten-list
                            (list (concat org-directory "active_things.org")
                                  ...
                                  "~/src/200ok/admin/ACTIVE_THINGS.org"
  ...
  (defun things ()
    ...
    (interactive)
    (find-file (concat org-directory "active_things.org"))
    (set-org-agenda-files)
    (org-agenda-list)
    (org-agenda-day-view)
...
Don't show habits outside of today's agenda view:

#+begin_src emacs-lisp
  (setq org-habit-show-habits-only-for-today t)
#+end_src

Include habits in today's agenda that are done:
#+begin_src emacs-lisp
  (setq org-habit-show-all-today t)
#+end_src

Move habit column to the far right.
...
#+end_src
*** Copy current Org path

Copy current Org path as: =filename.org / Parent / Child / Current
heading=. With prefix argument FULL-PATH, use the full file path
instead of just the file name.

#+begin_src emacs-lisp
  (defun ok/org-copy-current-path (&optional full-path)
    "Copy current Org path as: filename.org / Parent / Child / Current
  heading. With prefix argument FULL-PATH, use the full file path
  instead of just the file name."
    (interactive "P")
    ...
    (let* ((file (if buffer-file-name
           ...
           (headings
            (unless (org-before-first-heading-p)
              ;; t means include the current heading, not just parents.
              (mapcar #'substring-no-properties
                      (org-get-outline-path t))))
           (path (string-join (cons file headings) " / ")))
      (kill-new path)
...
** WIP Notifications / appointment reminders
 ...
 #+BEGIN_SRC emacs-lisp
   ...
   ;;           '(lambda ()
   ;;              (if (string= (buffer-file-name) (concat (getenv "HOME") "/Dropbox/org/active_things.org"))
   ;;                  (my-org-agenda-to-appt))))
...
#+begin_src emacs-lisp
    ...
    (defun munen-gptel--ddgs-search (query &optional max-results timelimit)
      "Search the web via the ddgs CLI for QUERY.
  MAX-RESULTS limits the number of results (default 10).
  TIMELIMIT restricts recency: \"d\" (day), \"w\" (week), \"m\" (month), \"y\" (year)."
      (with-temp-message (format "Searching ddgs for: `%s`" query)
        (condition-case err
             (let* ((json-file (make-temp-file "gptel-ddgs-" nil ".json"))
                    (args `("text"
                            "-q" ,query
                            "-m" ,(number-to-string (if (and (numberp max-results) (> max-results 0))
                                                        max-results 10))
                            "-r" "us-en"
                            "-s" "moderate"
                            "-b" "auto"
                            "-nc"
                            "-o" ,json-file)))
              (when (and timelimit (not (string-empty-p timelimit)))
                (setq args (append args (list "-t" timelimit))))
              (unwind-protect
                  (progn
                    (let ((exit-code (apply #'call-process "ddgs" nil nil nil args)))
                      (unless (zerop exit-code)
                        (error "ddgs exited with status %s" exit-code))
                      (with-temp-buffer
                               ...
                               (formatted (mapconcat
                                           (lambda (result)
                                             (format "%s - %s\n%s"
                                                     ...
                                                     (cdr (assoc 'href result))
                                                     (cdr (assoc 'body result))))
                                           results "\n\n")))
                          (if (string-empty-p formatted)
                              "No results found."
                            formatted)))))
                (delete-file json-file nil)))
          (error (format "Error searching with ddgs: %s" (error-message-string err))))))

    (gptel-make-tool
     :function #'munen-gptel--ddgs-search
     :name "ddgs_search"
     :description (concat
                   "Search the web using the ddgs metasearch CLI (DuckDuckGo-backed, aggregating multiple backends) "
                   "and return formatted results with titles, URLs and content excerpts. "
                   "Query operators like site:example.com, \"exact phrase\", -excluded and filetype:pdf are supported "
                   "directly in the query. Use the optional timelimit for recent results (news, releases).")
     :args (list
            '(:name "query"
                    :type string
                    :description "The search query. Prefer precise queries that disambiguate the subject.")
            '(:name "max-results"
                    :type integer
                    :optional t
            ...
            '(:name "timelimit"
                    :type string
                    :optional t
                    :description "Optional: Restrict results by recency: \"d\" (day), \"w\" (week), \"m\" (month), \"y\" (year). Omit for no restriction."))
     :category "web"
     :include t)
    ...
    (gptel-make-tool
     ...
     :description (concat
                   "Fetch content from a URL using trafilatura, which extracts main content and metadata "
                   "while removing boilerplate, navigation and ads. "
                   "Use this for articles, blog posts and news pages where you only want the main text. "
                   "It may return empty output on non-article pages (docs, dashboards, listings); "
                   "in that case, or when you need the complete page content, use the 'read_url' tool instead.")
     :args '((:name "url"
                     :type string
    ...
    (defun munen-gptel--read-url (url)
      "Fetch URL and render its full HTML contents as text using Emacs' built-in shr."
      (with-temp-message (format "Reading URL: %s" url)
        (condition-case err
            (with-current-buffer (url-retrieve-synchronously url t t)
              ...
              (let ((dom (libxml-parse-html-region (point) (point-max))))
                ...
                (with-temp-buffer
                  ...
                  (buffer-substring-no-properties (point-min) (point-max)))))
          (error (format "Error reading URL %s: %s" url (error-message-string err))))))

    (gptel-make-tool
     :name "read_url"
     :function #'munen-gptel--read-url
     :description (concat
                   "Fetch and read the contents of a URL by rendering the full page DOM as text "
                   "(via Emacs' built-in url-retrieve and shr). "
                   "This includes everything on the page: navigation, headers, footers and boilerplate. "
                   "Use this for non-article pages (documentation, listings, dashboards) or when you need "
                   "content that content extraction might strip (comments, sidebars, tables). "
                   "For articles or blog posts where you only want the main content in clean markdown, "
                   "prefer the 'TrafilaturaFetch' tool instead.")
     :args '((:name "url"
                     :type string
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'aidermacs
    ...
    (add-to-list 'aidermacs-extra-args "--analytics-disable")
  ...
  (use-package mcp
    ...
    :custom (mcp-hub-servers
             `(("playwright" . (:command "npx"
               ...
               ;;          :args ("-c"
               ;;                 "cd /home/munen/ && clojure -X:mcp :port 7890")))
               ))
    :config (require 'mcp-hub)
    ;; This works, but I don't want to always start all servers for all
    ;; instances of emacs.
...
**** Mail autocompletion
 ...
 last 6 months. This gets rid of legacy mail addresses of people.
  ...
  #+BEGIN_SRC emacs-lisp
    (defvar ok/mu4e-completion-blocklist
      '("sophia.benz@bos-schweiz.ch")
      "Email addresses that should not be offered by mu4e completion.")

    (defun ok/mu4e-compose-complete-handler-filter (orig-fun str pred action)
      "Filter blocklisted contacts before calling ORIG-FUN."
      (let ((blocked-regexp (regexp-opt ok/mu4e-completion-blocklist))
            (case-fold-search t))
        (funcall orig-fun str
                 (lambda (contact &rest args)
                   (and (not (string-match-p blocked-regexp contact))
                        (or (null pred) (apply pred contact args))))
                 action)))

    (unless (advice-member-p #'ok/mu4e-compose-complete-handler-filter
                             'mu4e--compose-complete-handler)
      (advice-add 'mu4e--compose-complete-handler
                  :around #'ok/mu4e-compose-complete-handler-filter))
...
#+begin_src emacs-lisp
  ...
  (setq gnus-icalendar-org-capture-file "~/Dropbox/org/active_things.org")
...
* ghost.el
https://dakra.github.io/ghostel/#quick-start
...
#+begin_src emacs-lisp
  (use-package ghostel
    ...
    :bind (("C-x m" . ghostel)
           ...
           ("M-p" . my/ghostel-history-previous)
           ("M-n" . my/ghostel-history-next)
           :map project-prefix-map
           ("m" . ghostel-project)
           ("M" . ghostel-project-list-buffers))
    :config
    (defun my/ghostel-send-C-k-and-kill ()
      "Kill to the end of the line and send C-k to Ghostel."
      (interactive)
      (kill-ring-save (point) (line-end-position))
      (ghostel-send-key "k" "ctrl"))

    (defun my/ghostel-history-previous ()
      "Send C-p to the terminal."
      (interactive)
      (ghostel-send-key "p" "ctrl"))

    (defun my/ghostel-history-next ()
      "Send C-n to the terminal."
      (interactive)
   ...
   #+begin_src emacs-lisp
     ...
     (setq org-agenda-custom-commands
           `(;; Agenda to only show personal GTD files and filter work
             ...
             ("h" "Habits Agenda"
              ((agenda ""))
              ((org-agenda-files (list (expand-file-name "habits.org" org-directory)))))
